...
export default function MovieDetails() {
    ...
    return (
        <div className="text-white flex flex-row gap-5 max-sm:my-10 max-sm:flex-col">
            <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} className='w-1/3 h-80 max-sm:w-full object-cover rounded-md' />
            <div className='flex flex-col gap-2'>
                <h1 className="text-3xl font-bold max-sm:text-lg">{movie.title}</h1>
                <p className='break-words'>{movie.overview}</p>
                <p className='text-yellow-700 max-sm:text-sm'>{movie.tagline}</p>
